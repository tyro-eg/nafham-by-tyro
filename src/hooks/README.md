...
**Mutation Function**: `(slots: any) => Promise<void>`
...
### Current Issues
Several hooks use `any` types that should be replaced with proper interfaces:

1. **Session Data** (`useBookTrialSession`, `useBookPrivateSession`):
   ```typescript
   // ❌ Current
...
2. **Availability Slots** (`useCreateSlots`):
   ```typescript
   // ❌ Current
...
3. **User Data** (`useUpdateUserInfo`):
   ```typescript
   // ❌ Current
   userData: any
   
   // ✅ Should be properly typed or deprecated
   ```
